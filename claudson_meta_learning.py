...
class MetaSGD(nn.Module):
    ...
    def __init__(self, model: nn.Module, args: ModelArgs):
        ...
        if args.maml_adapt_params == "heads":
            ...
            params = [
                (n, p) for n, p in model.named_parameters() if any(t in n for t in target_names)
            ]
        ...
        self.lrs = nn.ParameterList(
            [nn.Parameter(torch.ones_like(p.data) * args.meta_sgd_init) for _, p in params]
        )

        log.info("MetaSGD: %d parameter groups with learned learning rates", len(self.param_names))
    ...
    def forward(self) -> Dict[str, torch.Tensor]:
        return {n: lr.clamp(0.0, self.clip) for n, lr in zip(self.param_names, self.lrs)}
...
class ContextEncoder(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            self.task_emb_ema = (
                1 - self.ema_alpha
            ) * self.task_emb_ema + self.ema_alpha * z_task.detach().mean(0)
...
class InnerLoopAdapter(nn.Module):
    ...
    def compute_inner_loss(
        ...
        return self.inner_loss_head(torch.cat([pred, target], dim=-1)).mean()
...
class MetaValidationMonitor(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, n_steps: int, final_loss: float
    ) -> Tuple[torch.Tensor, Dict]:
...
class TaskConditionedForward(nn.Module):
    ...
    def forward(
        ...
        x_conditioned = self.norm(gamma.unsqueeze(1) * x + beta.unsqueeze(1))
    ...
    def adapt(
        ...
        for _ in range(self.inner_steps):
            ...
            out = functional_call(
                self.model,
                adapted,
                support_batch if isinstance(support_batch, tuple) else (support_batch,),
            )
            loss = loss_fn(out)
            ...
            adapted = {
                name: (
                    param - self.lr * grad.detach() if self.first_order else param - self.lr * grad
                )
                for (name, param), grad in zip(adapted.items(), grads)
                if grad is not None
            }
...
class ClaudesonMetaLearning(ClaudesonTemporalReasoning):
    ...
    def __init__(self, args: ModelArgs):
        ...
        log.info(
            "MetaLearning: %d parameters eligible for inner loop adaptation",
    ...
    def _identify_adapt_params(self, args: ModelArgs) -> List[str]:
        ...
        if args.maml_adapt_params == "heads":
            ...
            return [n for n, _ in self.named_parameters() if any(t in n for t in targets)]
        elif args.maml_adapt_params == "lora":
            return [
                n
    ...
    def _get_adapt_params(self) -> Dict[str, torch.Tensor]:
        ...
        return {n: p for n, p in self.named_parameters() if n in self._adapt_param_names}
    ...
    def meta_train_step(
        ...
        for task in task_batch:
            ...
            qx.mean(-1) if qx.dim() > 1 else qx
            # Proxy outer loss: distance between adapted and original
            outer_loss = sum(
                F.mse_loss(adapted[n], p.detach()) for n, p in adapt_params.items() if n in adapted
            )
...
if __name__ == "__main__":
    ...
    args.maml_inner_steps = 3
    args.maml_inner_lr = 0.01
    args.maml_first_order = True
    args.maml_adapt_params = "heads"
    args.meta_sgd = True
    ...
    args.bayes_n_particles = 3
    args.bayes_noise_std = 0.001
    args.mvm_window = 16
    args.mvm_ood_steps_thresh = 8
    args.n_shot = 3
    args.n_way = 3
