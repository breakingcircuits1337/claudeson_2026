...
class CausalDynamicsModel(nn.Module):
    ...
    def forward(
        ...
        if intervene and action is not None:
            if action.dim() == 1:
                action_oh = F.one_hot(
                    action, num_classes=self.action_intervention_proj[0].in_features
                ).float()
            else:
...
class CausalAttributionGate(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.register_buffer(
            "attribution_scores", torch.ones(args.n_causal_nodes) / args.n_causal_nodes
        )
    ...
    def update_attribution(self, credit: torch.Tensor) -> None:
        ...
        self.attribution_scores = self.ema * self.attribution_scores + (1 - self.ema) * batch_credit

    def forward(
        self, x: torch.Tensor, credit: Optional[torch.Tensor] = None
...
class ClaudesonCausalWorld(ClaudesonTranscendent):
    ...
    def forward(
        ...
        # ── Causal Dynamics ──────────────────────────────────────────────
        causal_out = self.causal_dynamics(
            x, action=actual_action, intervene=(actual_action is not None)
        )
        x = causal_out["hidden"]
    ...
    def compute_auxiliary_losses(self) -> Dict[str, torch.Tensor]:
        ...
        losses["causal_dynamics_dag"] = self.causal_dynamics.dag_constraint() * 0.01
        losses["planner_dag"] = self.interventional_planner.dynamics.dag_constraint() * 0.01
