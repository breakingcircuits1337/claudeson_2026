...
# Commercial — Generation G7. See COMMERCIAL_LICENSE for terms.
...
class PerceptualSemanticAnchor(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.anchors = nn.Parameter(torch.randn(args.psa_n_anchors, args.dim) * 0.02)
    ...
    def forward(
        ...
        sim_matrix = (
            torch.einsum(
                "bld,bsd->bls", F.normalize(x, dim=-1), F.normalize(sensory_context, dim=-1)
            )
            / self.temp
        )
...
class CrossModalAlignment(nn.Module):
    ...
    def forward(
        ...
        # Alignment loss
        align_loss = (
            sum(alignment_losses) / len(alignment_losses)
            if alignment_losses
...
class ClaudesonGroundedLanguage(ClaudesonUncertainty):
    ...
    def forward(
        ...
        concept_acts = (
            base.get("abstraction", {})
            .get("concept", {})
            .get(
                "activations",
                torch.zeros(x.size(0), x.size(1), self.args.n_concepts, device=x.device),
            )
        )
