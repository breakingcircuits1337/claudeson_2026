...
class BayesianUncertaintyPropagator(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.welfare_unc  = MCDropoutHead(args.n_stakeholder_groups, args.n_stakeholder_groups, h // 2, args.bup_dropout_rate)
        self.moral_unc    = MCDropoutHead(args.n_moral_frameworks,   args.n_moral_frameworks,   h // 2, args.bup_dropout_rate)
    ...
    def _update_epistemic_state(self, epistemic: torch.Tensor) -> None:
        self.epistemic_state = (
            self.unc_ema * self.epistemic_state
            + (1 - self.unc_ema) * epistemic.mean(0).mean(0)
        )
...
class CalibrationMonitor(nn.Module):
    ...
    def update(self, confidence: torch.Tensor, correct: torch.Tensor) -> None:
        ...
        if n > 0:
            ...
            conf = (torch.linspace(0, 1, self.n_bins, device=self.ece.device)
                    + 0.5 / self.n_bins)
...
class OODDetector(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.register_buffer(
            'centroids',
            torch.randn(args.ood_n_centroids, h) * 0.1
        )
        self.register_buffer(
            'centroid_counts',
            torch.ones(args.ood_n_centroids)
        )
    ...
    def update_centroids(self, features: torch.Tensor) -> None:
        ...
        for k in range(self.n_centroids):
            ...
            if mask.any():
                ...
                self.centroids[k] = (
                    self.ema * self.centroids[k]
                    + (1 - self.ema) * batch_mean
                )
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Dict]:
        ...
        # Combined OOD score
        ood_score = (mah_score / (mah_score.max() + 1e-6)
                     + energy_score + spectral_unc) / 3.0         # [B] ∈ [0,1]
...
class ClaudesonUncertainty(ClaudesonSocialAlignment):
    ...
    def forward(
        ...
        welfare    = sa.get("welfare", {}).get("welfare",
                       torch.zeros(x.size(0), self.args.n_stakeholder_groups, device=x.device))
        moral_scores = sa.get("moral", {}).get("framework_scores",
                       torch.zeros(x.size(0), self.args.n_moral_frameworks, device=x.device))
        moral_std    = sa.get("moral", {}).get("moral_std",
...
if __name__ == "__main__":
    ...
    for name, val in zip(unc['aggregated']['signal_names'],
                         unc['aggregated']['signals'][0].tolist()):
