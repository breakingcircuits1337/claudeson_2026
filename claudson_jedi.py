...
# Commercial — Generation G6. See COMMERCIAL_LICENSE for terms.
...
class SSDLayer(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        # Selective gate
        self.gate_fn = nn.Sequential(nn.Linear(args.dim, args.dim), nn.Sigmoid())
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B_ssm, C_ssm, delta_bias = torch.split(
            ssm_params, [self.state_dim, self.state_dim, self.n_heads], dim=-1
        )
...
class JediEnergyLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, action: Optional[torch.Tensor] = None) -> Dict:
        ...
        kl_loss = (
            -0.5 * precision.squeeze(-1) * (1 + logvar - prior_mu.pow(2) - logvar.exp()).mean(-1)
        )
    ...
    def imagination_rollout(
        self, initial_obs: torch.Tensor, action: torch.Tensor, horizon: int = 5
    ) -> Dict:
        """
...
class HierarchicalMemory(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        # Memory controllers
        self.write_gate = nn.Sequential(nn.Linear(args.dim * 2, args.dim), nn.Sigmoid())
    ...
    def forward(
        self, x: torch.Tensor, segment_boundary: bool = False
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        self.write_gate(torch.cat([segment_emb, self.working_mem], dim=-1))
        ...
        scores = torch.bmm(query.reshape(B * L, 1, D), k_exp.transpose(1, 2)).squeeze(1)  # [B*L, S]
        attn_weights = F.softmax(scores, dim=-1)  # [B*L, S]
        episodic_out = (
            torch.bmm(attn_weights.unsqueeze(1), v_exp).squeeze(1).view(B, L, D)
        )  # [B, L, D]
...
class HybridJediBlock(nn.Module):
    def __init__(self, args: ModelArgs, layer_idx: int):
        ...
        self.experts = nn.ModuleList(
            [SwiGLU(args.dim, args.dim * 4) for _ in range(args.num_experts)]
        )
    ...
    def forward(self, x: torch.Tensor, memory_bank, goal_cond=None, return_load_balance=False):
        ...
        if return_load_balance and self.training:
            ...
            expert_usage = (
                F.one_hot(top_k_idx[:, :, 0], num_classes=self.num_experts).float().mean(0)
            )
...
class ClaudesonJedi(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.vision_enc = nn.Conv2d(
            3, args.dim, kernel_size=args.patch_size, stride=args.patch_size
        )
        ...
        self.layers = nn.ModuleList([HybridJediBlock(args, i) for i in range(args.n_layers)])
        self.norm = RMSNorm(args.dim)

        self.monologue = nn.GRUCell(args.dim, args.dim)
        self.monologue_proj = nn.Sequential(
            SwiGLU(args.dim * 2, args.dim * 2), nn.Linear(args.dim * 2, args.dim), RMSNorm(args.dim)
        )
        ...
        self.value_head = nn.Sequential(
            SwiGLU(args.dim + args.goal_dim, args.dim), nn.Linear(args.dim + args.goal_dim, 1)
        )
