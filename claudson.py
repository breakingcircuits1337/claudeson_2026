...
class HierarchicalMemory(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.register_buffer(
            "episodic_memory", torch.randn(1, self.episodic_slots, compressed_dim) * 0.02
        )
        ...
        self.compressor = nn.Sequential(
            nn.Linear(args.dim, args.dim // 2), nn.GELU(), nn.Linear(args.dim // 2, compressed_dim)
        )
        self.decompressor = nn.Sequential(
            nn.Linear(compressed_dim, args.dim // 2), nn.GELU(), nn.Linear(args.dim // 2, args.dim)
        )
    ...
    def consolidate(self, working_mem: torch.Tensor):
        ...
        if important.size(0) > 0:
            ...
            self.episodic_memory.data = 0.99 * self.episodic_memory + 0.01 * compressed.mean(
                0, keepdim=True
            )
    ...
    def retrieve_contextual(self, query: torch.Tensor) -> torch.Tensor:
        ...
        episodic_scores = torch.bmm(query, episodic_decompressed.transpose(1, 2)) / math.sqrt(
            query.size(-1)
        )
        ...
        semantic_scores = torch.bmm(query, semantic_expanded.transpose(1, 2)) / math.sqrt(
            query.size(-1)
        )
...
class HybridBlock(nn.Module):
    ...
    def forward(
        self,
        x: torch.Tensor,
        memory_bank: HierarchicalMemory,
        goal_cond: Optional[torch.Tensor] = None,
        kv_cache: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
    ) -> Tuple[
        torch.Tensor, torch.Tensor, torch.Tensor, Optional[Tuple[torch.Tensor, torch.Tensor]]
    ]:
        res = x
        x = self.norm1(x)
...
class ImprovedWorldModel(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.state_proj = nn.Linear(
            args.dim + args.action_space_size + args.env_state_dim, args.dim
        )
    ...
    def predict_step(
        self, h: torch.Tensor, action: torch.Tensor, env_state: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        action_onehot = F.one_hot(
            action, num_classes=action.size(-1) if action.dim() > 1 else self.action_space_size
        ).float()
...
class TreeSearchPlanner(nn.Module):
    ...
    def _simulate_trajectory(
        self, state: torch.Tensor, goal: torch.Tensor, env_state: torch.Tensor
    ):
        trajectory = []
        h = state
        curr_env = env_state
...
class InternalMonologue(nn.Module):
    ...
    def forward(
        ...
        prev_thought: Optional[torch.Tensor],
    ):
        B, L, D = x.shape
        pooled = x.mean(dim=1)
...
class ConstitutionalLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        principle_pull = torch.einsum("blp,pd->bld", deficit, self.principle_vectors)  # [B, L, D]
...
class UniversalIntelligenceModel(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.constitutional = ConstitutionalLayer(
            args.dim, steer_scale=args.constitutional_steer_scale
        )
    ...
    def forward(
        ...
        agency_results = self.agency(
            ...
            goal_cond
            if goal_cond is not None
            else torch.zeros(B, 1, self.args.goal_dim, device=x.device),
            env_state,
        )
...
class TrainingConfig:
    ...
    def get_optimizer(model, lr=1e-4, weight_decay=0.1):
        ...
        for layer_id, layer in enumerate(model.layers):
            ...
            param_groups.append(
                {"params": layer.parameters(), "lr": lr * decay_rate, "weight_decay": weight_decay}
            )
...
class MultiModalDataset(torch.utils.data.Dataset):
    ...
    def _load_data(self):
        ...
        if os.path.isfile(self.data_path):
            ...
            files = glob.glob(os.path.join(self.data_path, "*.pt")) + glob.glob(
                os.path.join(self.data_path, "*.pth")
            )
    ...
    def __call__(self, batch):
        ...
        if "text" in batch[0]:
            ...
            text_batch = torch.stack(
                [
                    F.pad(
                        item["text"], (0, max_len - item["text"].size(0)), value=self.pad_token_id
                    )
                    for item in batch
                ]
            )
        ...
        if "audio" in batch[0]:
            ...
            audio_batch = torch.stack(
                [F.pad(item["audio"], (0, 0, 0, max_len - item["audio"].size(0))) for item in batch]
            )
...
class InferenceEngine:
    ...
    def batch_inference(
        ...
        padded_text = torch.stack([F.pad(t, (0, max_len - t.size(0))) for t in batch_text]).to(
            self.device
        )
        ...
        with torch.cuda.amp.autocast():
            outputs = self.model(text=padded_text, img=batch_imgs, goal=batch_goals)
