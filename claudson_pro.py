...
class SharedExpertMoE(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.experts = nn.ModuleList(
            [
                nn.Sequential(
                    SwiGLU(args.dim, args.dim * 4), nn.Linear(args.dim, args.dim, bias=False)
                )
...
class TransformerWorldModel(nn.Module):
    ...
    def forward(
        ...
        return {"next_state": next_state, "reward": reward, "uncertainty": uncertainty}
...
class HierarchicalMemory(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.compressor = nn.Sequential(
            nn.Linear(args.dim, args.dim // 2), nn.SiLU(), nn.Linear(args.dim // 2, compressed_dim)
        )
        self.decompressor = nn.Sequential(
            nn.Linear(compressed_dim, args.dim // 2), nn.SiLU(), nn.Linear(args.dim // 2, args.dim)
        )
...
class InternalMonologuePro(nn.Module):
    ...
    def __init__(self, dim: int, steps: int = 5):
        ...
        self.thought_proj = nn.Sequential(
            SwiGLU(dim * 2, dim * 2), nn.Linear(dim * 2, dim), RMSNorm(dim)
        )
...
class UniversalIntelligenceModelPro(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.value_head = nn.Sequential(
            SwiGLU(args.dim + args.goal_dim, args.dim), nn.Linear(args.dim, 1)
        )
