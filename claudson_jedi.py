...
class JediEnergyLayer(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        # Decoder: latent + action → prediction
    ...
    def planning(self, initial_obs: torch.Tensor, horizon: int = 5) -> torch.Tensor:
        ...
        # Sample actions to try (use action_space_size from args)
        num_actions = min(10, self.action_space_size)
        action_range = torch.arange(num_actions, device=z.device)
        
        # Try all actions and compute total energy
        best_energy = float('inf') * torch.ones(B, device=z.device)
        best_actions = torch.zeros(B, dtype=torch.long, device=z.device)
        
        for a in action_range:
            action = action_range.new_full((B,), a)
            latent = z.clone()
            ...
            # Update best actions where this action is better
            is_better = total_energy < best_energy
            best_energy = torch.where(is_better, total_energy, best_energy)
...
class HybridJediBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, memory_bank, goal_cond=None, jedi_output=None):
        ...
        # MoE - vectorized implementation
        gate_logits = self.gate(self.norm2(x))
        top_k_logits, top_k_idx = torch.topk(gate_logits, self.top_k, dim=-1)
        
        # Compute weights for top-k experts
        weights_topk = F.softmax(top_k_logits, dim=-1).unsqueeze(-1)  # [B, L, top_k, 1]
        ...
        for i in range(self.top_k):
            expert_idx = top_k_idx[:, :, i]  # [B, L]
            expert_weights = weights_topk[:, :, i, :]  # [B, L, 1]
            
            # Gather expert outputs - vectorized
