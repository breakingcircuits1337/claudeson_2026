...
class DynamicRouter(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, mode_override: Optional[int] = None
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if L <= self.window_size:
            scores = scores + self.rel_pos_bias[
                self.window_size : self.window_size + L, :L
            ].unsqueeze(0)
...
class ChunkedSSM(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.A_log = nn.Parameter(
            torch.log(torch.arange(1, self.state_dim + 1).repeat(args.dim, 1).float())
        )
...
class PagedMemorySystem(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.page_index = nn.Parameter(torch.zeros(self.num_pages, dtype=torch.long))
    ...
    def page_out(self, x: torch.Tensor):
        ...
        self.episodic_compressed.data = torch.roll(self.episodic_compressed.data, shifts=-1, dims=1)
...
class InfiniteContextBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mode_override: Optional[int] = None):
        ...
        mixed = (
            w[:, 0:1] * attn_out + w[:, 1:2] * ssm_out + w[:, 2:3] * conv_out + w[:, 3:4] * mem_out
        )
...
class InfiniteContextModel(nn.Module):
    ...
    def forward(
        self, text=None, img=None, audio=None, goal=None, mode_override: Optional[int] = None
    ):
        B = 0
        tokens = []
